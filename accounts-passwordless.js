...
if (Meteor.isClient) {
  ...
  Meteor.sendVerificationCode = function (selector, options, callback) {
    ...
    // Save the selector in a sessionStorage so even if the client reloads, the
    window.sessionStorage.setItem('accounts-passwordless.selector', selector);
  ...
  Meteor.loginWithPasswordless = function (options, callback) {
    ...
    Accounts.callLoginMethod({
      methodArguments: [{
        selector: window.sessionStorage.getItem('accounts-passwordless.selector') || options.selector,
        code: options.code
      }],
